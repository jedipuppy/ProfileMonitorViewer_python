...
class ApplicationWindow(QMainWindow):
	...
	def initUI(self):
		...
		save_preset = QAction("Save Preset", self)
		save_preset.setShortcut("Ctrl+S")
		fileMenu.addAction(save_preset)
		load_preset = QAction("Load Preset", self)
		load_preset.setShortcut("Ctrl+L")
		fileMenu.addAction(load_preset)
...
class VolConsole(QWidget):
	...
	def init_ui(self):
		...
		vol_list = np.array([\
		["VC",201],			["VA",202],			["",0],				["",0],		["",0],		["",0],
		["VL1p",201],		["VL1m",201],		["VL2p",0],			["VL2m",0],	["ST1p",0],	["ST1m",0],	
		["Dp",201],			["Dm",201],			["",0],				["",0],		["",0],		["",0],
		["ST2p",201],		["ST2m",201],		["Q1ACp",0],		["Q1ACm",0],["",0],		["",0],
		["BPM1F",201],		["BPM1B",201],		["BPM1P",0],		["",0],		["",0],		["",0]
		])



		vol_names =vol_list[:,0]
		vol_assigns =vol_list[:,1]


		grid = QGridLayout()
		self.setLayout(grid)
		positions = [(i,j) for i in range(6) for j in range(6)]
		for position, name, assign in zip(positions, vol_names, vol_assigns):
			print("read "+str(name))
			...
			exec("grid.addWidget(self.vol_"+name+", *position)")


	def keyPressEvent(self, e):
		print(vol_select)
		if vol_select != "": 
			print("self.vol_"+vol_select+".vol_set.text=1")	
			exec("current_vol = float(self.vol_"+vol_select+".vol_set.text())")
			vol_diff = 0			
			if e.key() == Qt.Key_W : 
				vol_diff = 1
			elif e.key() == Qt.Key_S : 
				...
				vol_diff = 0.1
			exec("self.vol_"+vol_select+".vol_set.setText(str(round(current_vol+vol_diff,1)))")   
...
class VolPart(QWidget):
	...
	def init_ui(self,name,assign):
		...
		self.vol_name = QPushButton(name)

		self.vol_name.clicked.connect(self.vol_click)	
		self.vol_set = QLineEdit("0")
		self.vol_read = QLabel('10 V')
		self.cur_leak = QLabel('3 mA')
		self.vol_box.addWidget(self.vol_name,1)
	...
	def vol_click(self):
		global vol_select
		self.vol_name.setFont(self.bold)
		vol_select = self.name
		print(vol_select)
